...
interface EditOperationalGoalProps {
...
export default function EditOperationalGoal({ goal, open, onOpenChange, onSuccess }: EditOperationalGoalProps) {
    ...
    const form = useForm<OperationalGoalFormValues>({
        ...
        defaultValues: {
            ...
            trackingMethod: goal.trackingMethod || 'weekly',
            weight: goal.excludeFromCalculation ? '' : String(goal.weight || ''),
            excludeFromCalculation: goal.excludeFromCalculation || false,
            isReverse: goal.isReverse || false,
            calculationMethod: (goal as any).calculationMethod || '',
            displayOnGeneral: (goal as any).displayOptions?.includes('general') || false,
            displayOnOperational: (goal as any).displayOptions?.includes('operational') || false,
            icon: goal.icon || 'fa-tasks',
            periods: [],
        },
    });
    ...
    useEffect(() => {
        if (goal && goal.progress) {
            const periods = goal.progress.map((period: PeriodProgress) => ({
                year: period.year,
                quarter: period.quarter,
                target: period.target || 0,
                achieved: period.achieved || period.weeklyTotalAchieved || 0,
    ...
    const onSubmit = async (data: OperationalGoalFormValues) => {
        ...
        try {
            ...
            const progress = data.periods.map((period) => {
                const existingPeriod = goal.progress?.find(
                ...
                if (data.trackingMethod === 'weekly') {
                    periodData.weeks =
                        existingPeriod?.weeks && existingPeriod.weeks.length > 0
                            ? existingPeriod.weeks
                            : Array.from({ length: 13 }, (_, i) => ({ week: i + 1, achieved: 0 }));
                    periodData.weeklyTotalAchieved =
                        existingPeriod?.weeklyTotalAchieved !== undefined ? existingPeriod.weeklyTotalAchieved : 0;
                } else {
                    // Direct entry
                    periodData.achieved =
                        existingPeriod && existingPeriod.achieved !== undefined
                            ? existingPeriod.achieved
                            : period.achieved || 0;
                }
            ...
            console.error('Error updating operational goal:', error);
            showToast('حدث خطأ أثناء تحديث الهدف', 'error');
        } finally {
                                                ...
                                                >
                                                    ...
                                                    >
                                                        {strategicGoals.map((g) => {
                                                            const yearsDisplay = Array.isArray(g.years)
                                                                ? g.years.join(', ')
                                                                : '';
                                                            return (
                                                                <SelectItem key={g.id} value={g.id}>
                                                                    {g.goal} {yearsDisplay ? `(${yearsDisplay})` : ''}
                                                                </SelectItem>
                                                            );
                                                        })}
                                                    </SelectContent>
